  ...
  theme: {
    extend: {
      colors: {
        ...
        // design-system v2 addition, safe to ignore for plain Tier 1 UI
      ...
      borderRadius: {
        ...
        DEFAULT: "var(--radius)",
        brand: "var(--radius)",
        // slightly larger radius for hero/flagship panels (design-system v2)
        xl2: "calc(var(--radius) * 1.5)",
      },
      maxWidth: {
        // matches --max-width / the static template's `.wrap` container
        site: "var(--max-width)",
        // wider container for dashboard/bento layouts — opt in per page
        "site-wide": "1120px",
      },
      fontFamily: {
        sans: ["var(--font-sans)"],
      ...
      // --- Design-system v2 additions (Tier 1/2/3, see
      // _shared/nextjs-template/DESIGN_SYSTEM.md) — everything above this
      // point is untouched so existing UI keeps rendering identically. ---
      fontSize: {
      ...
      backgroundImage: {
        // radial mesh built from the two accent hues — used by
        // components/motion/GradientMesh.tsx as a decorative backdrop
        "gradient-mesh":
