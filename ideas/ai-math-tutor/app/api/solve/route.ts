...
import {
  ...
  parseMathTutorResponse,
  validateMathTutorInput,
} from "@/lib/mathtutor";
...
export async function POST(request: Request) {
  ...
  const validated = validateMathTutorInput(body);
  if (!validated.ok) {
    return NextResponse.json(
  ...
  const prompt = buildMathTutorPrompt(validated.value);

  let raw;
  try {
    raw = await callGemini(prompt, { temperature: 0.2 });
  } catch (err) {
  ...
  const parsed = parseMathTutorResponse(raw);
  if (!parsed.ok) {
    return NextResponse.json(
